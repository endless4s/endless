...
Pekko runtime is available in `endless-runtime-pekko` (for Akka, use `endless-runtime-akka`). There are also helpers for defining protobuf protocols in `endless-protobuf-helpers` and circe in `endless-circe-helpers`. Add those dependencies to the project where your application wiring code resides (typically "infrastructure").
